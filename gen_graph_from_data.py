...
# Subsustems instantiation
communication = SubSysType('communication', [
                           slug, ontology, power, mass, min_working_temp, max_working_temp])

propulsion = SubSysType('propulsion', [
                        slug, ontology, power, mass, cost, min_working_temp, max_working_temp])

detector = SubSysType('detector', [
                      slug, ontology, power, mass, cost, min_working_temp, max_working_temp])

primary_power = SubSysType('primary power', [
                           slug, ontology, power, density, mass, cost, min_working_temp, max_working_temp])

backup_power = SubSysType('backup power', [
                          slug, ontology, power, density, mass, cost, min_working_temp, max_working_temp])

thermal = SubSysType('thermal', [
                     slug, ontology, power, mass, cost, min_working_temp, max_working_temp])

structure = SubSysType(
    'structure', [slug, ontology, mass, cost, min_working_temp, max_working_temp])

command_and_data = SubSysType('command and data', [
                              slug, ontology, power, mass, cost, min_working_temp, max_working_temp])
...
def gen_components():
    ...
    for resource in data:
        # print(resource['name'])
        component_list.append(Resource(name=resource['name'], type_= sub_system_mapping[resource['object']['category']]))
...
def find_property_value(object_name, property_name):
    """ Finds a property in data object provided object name and property_name. """
    result_obj = None
    for object_ in data:
        # print(object_['name'])
...
def gen_graph_data(component_list):
    ...
    for component in component_list:
        for attribute in component.type_.allowed_properties:
            ...
            graph_data.append(Graph(component, attribute, find_property_value(component.name, attribute.name)))
    return graph_data
...
for graph_object in graph_data:
    print(graph_object.subject.name, graph_object.predicate.name, graph_object.object)
